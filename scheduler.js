...
(function(){
    ...
    getAllPeople(state).forEach(person => {
      ...
      weeks.forEach(week => {
        ...
        avail[person.id][week.weekStart] = {
          serviceAvailable: !hasVacation(state, person.id, week.weekStart, week.weekEnd),
          nightAvailable: !hasVacation(state, person.id, week.weekStart, week.weekEnd) && !hasRequest(state, person.id,'no_call',week.weekStart, week.weekEnd) && !hasRequest(state, person.id,'no_night_call',week.weekStart, week.weekEnd),
  ...
  function assignMajorServices(state, weeks, availability, options={}){
    ...
    // BB ICU rescue pass (surgical): after the normal schedule is built, make sure BB
    // receives about 80% of a normal full-time ICU share. To satisfy the local rule,
    // any ICU week given to BB is also made a BB Resp week. This is intentionally a
    // small post-pass and does not change the main fairness engine.
    (function ensureBBGetsLimitedIcuOnRespWeeks(){
      const bb = state.physicians.find(p => p.id === 'BB');
      if(!bb || !canDoService(bb, 'ICU') || !canDoService(bb, 'Resp')) return;

      function countService(ownerId, service){
        return weeks.reduce((sum, week) => sum + ((week.services || {})[service] === ownerId ? 1 : 0), 0);
      }
      function decrementService(ownerId, service){
        if(!ownerId || !store[ownerId]) return;
        store[ownerId].serviceCounts = store[ownerId].serviceCounts || {};
        store[ownerId].serviceCounts[service] = Math.max(0, (store[ownerId].serviceCounts[service] || 0) - 1);
        const person = physicianMap(state)[ownerId];
        if(service === 'ICU'){
          store[ownerId].icuWeeks = Math.max(0, (store[ownerId].icuWeeks || 0) - 1);
          store[ownerId].weighted.icu = +(store[ownerId].icuWeeks / fairnessWeight(person,'icu')).toFixed(2);
        } else {
          store[ownerId].weeklyServices = Math.max(0, (store[ownerId].weeklyServices || 0) - 1);
          store[ownerId].weighted.services = +(store[ownerId].weeklyServices / fairnessWeight(person,'service')).toFixed(2);
        }
      }

      const fullTimeIcuIds = state.physicians
        .filter(p => p.id !== 'BB' && p.id !== 'DC' && canDoService(p, 'ICU'))
        .map(p => p.id);
      const avgFullTimeIcu = fullTimeIcuIds.length
        ? fullTimeIcuIds.reduce((sum, id) => sum + countService(id, 'ICU'), 0) / fullTimeIcuIds.length
        : 0;
      const target = Math.max(1, Math.round(avgFullTimeIcu * 0.8));
      let current = countService('BB', 'ICU');
      if(current >= target) return;

      const candidates = weeks.filter(week => {
        if(week.locked) return false;
        if((week.pinnedServices || {}).ICU || (week.pinnedServices || {}).Resp) return false;
        if((week.services || {}).ICU === 'BB') return false;
        if(!availability.BB?.[week.weekStart]?.serviceAvailable) return false;
        if(availability.BB?.[week.weekStart]?.noIcuWeek) return false;
        const assigned = peopleAssignedThisWeek(week);
        const bbOtherService = assigned.BB;
        if(bbOtherService && !['Resp','ICU'].includes(bbOtherService)) return false;
        return true;
      }).sort((a,b) => {
        const aAlreadyResp = (a.services || {}).Resp === 'BB' ? 0 : 1;
        const bAlreadyResp = (b.services || {}).Resp === 'BB' ? 0 : 1;
        if(aAlreadyResp !== bAlreadyResp) return aAlreadyResp - bAlreadyResp;
        const aOldIcu = (a.services || {}).ICU || '';
        const bOldIcu = (b.services || {}).ICU || '';
        const aOldCount = aOldIcu && store[aOldIcu] ? (store[aOldIcu].serviceCounts?.ICU || 0) : 0;
        const bOldCount = bOldIcu && store[bOldIcu] ? (store[bOldIcu].serviceCounts?.ICU || 0) : 0;
        return bOldCount - aOldCount || a.weekStart.localeCompare(b.weekStart);
      });

      for(const week of candidates){
        if(current >= target) break;
        const oldResp = (week.services || {}).Resp || '';
        const oldIcu = (week.services || {}).ICU || '';
        if(oldResp && oldResp !== 'BB') decrementService(oldResp, 'Resp');
        if(oldIcu && oldIcu !== 'BB') decrementService(oldIcu, 'ICU');
        week.services.Resp = 'BB';
        week.services.ICU = 'BB';
        if(oldResp !== 'BB') addAssignmentChargeSimple(store, state, 'BB', 'service', 'Resp');
        addAssignmentChargeSimple(store, state, 'BB', 'service', 'ICU');
        current = countService('BB', 'ICU');
      }
    })();
  ...
  function explainUnfilledRequiredService(state, weeks, week, service, availability){
    const assigned = peopleAssignedThisWeek(week);
    return (state.physicians || []).map(p => {
      const reasons = [];
      if(!canDoService(p, service)) reasons.push('not eligible for ' + service);
      if(assigned[p.id]) reasons.push('already assigned ' + assigned[p.id]);
      const av = availability?.[p.id]?.[week.weekStart];
      if(!av || av.serviceAvailable === false){
  ...
  function validateSchedule(state, weeks, nights){
    ...
    weeks.forEach(week => {
      ...
      MAJOR_SERVICES.forEach(service => {
        const owner = week.services[service];
        if(!owner){
          if(!OPTIONAL_OP_SERVICES.includes(service)){
            const details = explainUnfilledRequiredService(state, weeks, week, service, availability);
            issues.push({ level:'warning', code:'service-unfilled-debug', message:`${service} unfilled for ${week.weekStart}. Rule audit: ${details}`, weekStart:week.weekStart });
          }
          return;
        }
