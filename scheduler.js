...
(function(){
  ...
  function addFairnessCharge(state, store, ownerId, kind, service){
    ...
    if(kind === 'weekend'){
      ...
      if(service){
        store[targetId].serviceCounts = store[targetId].serviceCounts || {};
        store[targetId].serviceCounts[service] = (store[targetId].serviceCounts[service] || 0) + 1;
      }
      if(service === 'ICU'){
  ...
  function assignMajorServices(state, weeks, availability, options={}){
    ...
    // Surgical BB ICU correction:
    // After Resp and ICU are generated normally, top BB up to his 0.8 ICU target
    // by swapping ICU onto weeks where BB already has Resp. This does not force Resp.
    (function ensureBBIcuTargetOnRespWeeks(){
      const bb = state.physicians.find(p => p.id === 'BB');
      if(!bb || !canDoService(bb, 'ICU')) return;
      const target = Math.max(1, Math.round(icuTargets.BB || 0));
      let current = store.BB?.serviceCounts?.ICU || 0;
      if(current >= target) return;
      const candidates = weeks.filter(week => {
        if(week.locked) return false;
        if(week.services?.Resp !== 'BB') return false;
        ...
        const aOwner = a.services?.ICU || '';
        const bOwner = b.services?.ICU || '';
        const aCount = aOwner && store[aOwner] ? (store[aOwner].serviceCounts?.ICU || 0) : 0;
        const bCount = bOwner && store[bOwner] ? (store[bOwner].serviceCounts?.ICU || 0) : 0;
        return bCount - aCount || a.weekStart.localeCompare(b.weekStart);
      });
      for(const week of candidates){
        if(current >= target) break;
        const oldOwner = week.services?.ICU || '';
        if(oldOwner && store[oldOwner]){
          store[oldOwner].icuWeeks = Math.max(0, (store[oldOwner].icuWeeks || 0) - 1);
          if(store[oldOwner].serviceCounts){
            store[oldOwner].serviceCounts.ICU = Math.max(0, (store[oldOwner].serviceCounts.ICU || 0) - 1);
          }
          const oldPerson = physicianMap(state)[oldOwner];
          store[oldOwner].weighted.icu = +(store[oldOwner].icuWeeks / fairnessWeight(oldPerson,'icu')).toFixed(2);
        }
        week.services.ICU = 'BB';
        addAssignmentChargeSimple(store, state, 'BB', 'service', 'ICU');
        current = store.BB?.serviceCounts?.ICU || 0;
      }
    })();
